...
void async_io_manager::initialize(size_t thread_count) {
    ...
    if (!cfg_cache_folder.is_empty()) {
        ...
        // Use default: profile path + foo_artgrab_data\_cache
        ...
        cache_dir << "\\foo_artgrab_data\\_cache\\";
    }
        ...
        workers.emplace_back([this] {
            for (;;) {
                ...
                try {
                    ...
                    pfc::string8 error_msg = "foo_artgrab: Thread pool task exception: ";
                    error_msg << e.what();
                    fb2k::inMainThread([error_msg]() {
                        console::print(error_msg);
                    });
                } catch (...) {
                    fb2k::inMainThread([]() {
                        console::print("foo_artgrab: Unknown thread pool exception");
                    });
                }
            }
        });
...
void async_io_manager::async_cache::write_worker() {
    while (!shutdown_requested) {
        ...
        if (!write_queue.empty()) {
            ...
            // Perform synchronous write directly (we're on a background thread,
            // so we can't call write_file_async which asserts main thread)
            pfc::string8 file_path = get_cache_file_path(item.first);
            std::wstring wide_file_path = utf8_to_wide(file_path);

            // Create directory if needed
            pfc::string8 directory = file_path;
            t_size dir_pos = directory.find_last('\\');
            if (dir_pos != pfc_infinite) {
                directory.truncate(dir_pos);
                std::wstring wide_dir = utf8_to_wide(directory);
                SHCreateDirectoryExW(nullptr, wide_dir.c_str(), nullptr);
            }

            HANDLE file = CreateFileW(wide_file_path.c_str(), GENERIC_WRITE, 0, nullptr,
                CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
            if (file != INVALID_HANDLE_VALUE) {
                DWORD written;
                WriteFile(file, item.second.get_ptr(),
                    static_cast<DWORD>(item.second.get_size()), &written, nullptr);
                CloseHandle(file);
            }
        }
    }
...
void async_io_manager::async_cache::clear_all() {
    ...
    instance().submit_task([cache_dir]() {
        ...
        fb2k::inMainThread([]() {
            console::print("foo_artgrab: Artwork cache cleared.");
        });
    });
}
...
void async_io_manager::main_thread_dispatcher::initialize() {
    ...
    WM_ASYNC_CALLBACK = RegisterWindowMessageA("foo_artgrab_async_callback");

    WNDCLASSEXA wc = {};
    wc.cbSize = sizeof(wc);
    wc.lpfnWndProc = window_proc;
    wc.hInstance = GetModuleHandle(nullptr);
    wc.lpszClassName = "foo_artgrab_async_dispatcher";
    RegisterClassExA(&wc);

    message_window = CreateWindowExA(
        0, "foo_artgrab_async_dispatcher", "",
        0, 0, 0, 0, 0,
...
static bool perform_http_get_internal(const pfc::string8& url, pfc::string8& response, pfc::string8& error_message, int timeout_seconds) {
    ...
    // Initialize WinHTTP
    HINTERNET hSession = WinHttpOpen(L"foobar2000-artgrab/1.0",
...
void async_io_manager::perform_http_get(const pfc::string8& url, http_request_callback callback) {
    ...
    int timeout_seconds = cfg_ag_http_timeout.get_value();
    int max_retries = cfg_ag_retry_count.get_value();
    ...
    for (int attempt = 0; attempt <= max_retries; attempt++) {
        ...
        if (attempt < max_retries) {
            console::printf("foo_artgrab: HTTP request failed (attempt %d/%d), retrying: %s",
...
void async_io_manager::perform_http_get_binary(const pfc::string8& url, file_read_callback callback) {
    ...
    for (int attempt = 0; attempt <= max_retries; attempt++) {
        ...
        // Log retry attempt
        if (attempt < max_retries) {
            console::printf("foo_artgrab: Binary HTTP request failed (attempt %d/%d), retrying: %s",
                           attempt + 1, max_retries + 1, error_message.c_str());
        }
    }
